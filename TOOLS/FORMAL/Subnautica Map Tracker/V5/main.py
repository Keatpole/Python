...
map_img_1 = pygame.image.load("map1.png")
map_img_2 = pygame.image.load("map2.png")

map_img_1 = pygame.transform.scale(map_img_1, (720, 720))
map_img_2 = pygame.transform.scale(map_img_2, (720, 720))

screen = pygame.display.set_mode((map_img_1.get_width(), map_img_1.get_height()))
...
selected = None
selected_color = (0, 255, 0, 100) # Change this to the color the selected color should be (RGBA)

log_from_file = []
log = []

# Set map_img_1's alpha values to 0
alphas = pygame.surfarray.pixels_alpha(map_img_1)
...
def anim(t):
    ...
    for i,l in enumerate(log_from_file):
        screen.blit(map_img_2, (0, 0))
        screen.blit(map_img_1, (0, 0))

        pixels = pygame.PixelArray(map_img_1)

        if selected:
            pixels.replace(selected_color, selected)

        pixels.replace(l + (0,), selected_color)

        del pixels

        selected = l + (selected_color[3],)

        pygame.display.update(map_img_1.get_rect())
    ...
    if selected:
        ...
        pixels.replace(selected, selected_color)

        del pixels

        selected = log_from_file[-1] + (selected_color[3],)
...
while True:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if event.button == 1:
                color = map_img_1.get_at(pygame.mouse.get_pos())
