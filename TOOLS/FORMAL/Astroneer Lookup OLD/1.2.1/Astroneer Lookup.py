...
ALL_RESOURCES["BUILDINGS"] = {
    ...
    "MEDIUM WIND TURBINE": Object("MEDIUM WIND TURBINE", ALL_RESOURCES["SMELTED_RESOURCES"]["ALUMINUM"], ALL_RESOURCES["SMELTED_RESOURCES"]["CERAMIC"], bytes=2500, tier=2),
...
def get_item_info(tab, times, item, tier=False):
    if tier: return f"{'    ' * tab}{times}{item.name} ({item.type} | Tier {item.tier}):\n"
    else: return f"{'    ' * tab}{times}{item.name} ({item.type}):\n"

def lookup(item, tab=1, times=""):

    if isinstance(item, Resource): # Is Resource
        return f"{get_item_info(tab-1, times, item)}{'    ' * tab}Planets: {', '.join(item.planets)}"
    
    elif isinstance(item, RefinedResource): # Is Smelted Resource
        return f"{get_item_info(tab-1, times, item)}{lookup(item.smeltedFrom, tab+1)}"
    
    elif isinstance(item, AtmoResource): # Is Atmospheric Resource (aka Gas)
        atmo = ""
        for i in item.planetsAndPPU:
            atmo += f"{'    ' * tab}{i} ({item.planetsAndPPU[i]} PPU),\n"
        return f"{get_item_info(tab-1, '', item)}{atmo[:-2]}"

    elif isinstance(item, CompResource): # Is Composite Resource
        if item.res1 == item.res2:
            info = f"{get_item_info(tab-1, times, item)}{lookup(item.res1, tab+1, 'x2 ')}"
        else:
            info = f"{get_item_info(tab-1, times, item)}{lookup(item.res1, tab+1)}\n{lookup(item.res2, tab+1)}"
        ...
        if item.res1 == item.res2:
            if item.res2 == item.res3:
                if item.res3 == item.res4:
                    info = f"{get_item_info(tab-1, times, item, True)}{lookup(item.res1, tab+1, 'x4 ')}"
                else:
                    info = f"{get_item_info(tab-1, times, item, True)}{lookup(item.res1, tab+1, 'x3 ')}"
                    if item.res4:
                        info += f"\n{'    '*(tab-1)}{lookup(item.res4, tab+1)}"
            else:
                info = f"{get_item_info(tab-1, times, item, True)}{lookup(item.res1, tab+1, 'x2 ')}"
                if item.res3:
                    if item.res3 == item.res4:
            ...
            info = f"{get_item_info(tab-1, times, item, True)}{lookup(item.res1, tab+1)}"
            if item.res3 == item.res4:
                info = f"{get_item_info(tab-1, times, item, True)}{lookup(item.res3, tab+1, 'x3 ')}"
            else:
                info += f"\n{'    '*(tab-1)}{lookup(item.res3, tab+1, 'x2 ')}"
                if item.res4:
                    info += f"\n{'    '*(tab-1)}{lookup(item.res4, tab+1)}"
...
while True:
    ...
    if req.startswith("/"): # Commands
        ...
        if req == "/items": # Display all items
            ...
            for i in ALL_RESOURCES:
                ...
                elif i.startswith("COMP_RESOURCES"):
            ...
            for i in ALL_RESOURCES:
                if i.startswith("BUILDINGS"):
                    for v in ALL_RESOURCES[i]:
                        if ALL_RESOURCES[i][v].tier == tier:
                            print(f"{v.lower().capitalize()}")
            print("\n")
    ...
    try:
        print(f"RESOURCE FOUND!\n\n{lookup(ALL_RESOURCES[[res for res in ALL_RESOURCES if req in ALL_RESOURCES[res]][0]][req])}\n")
    except:
